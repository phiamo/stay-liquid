...
type TabBarMinimizeBehavior = "never" | "onScrollDown" | "onScrollUp" | "automatic";
...
interface BottomAccessoryOptions {
...
interface TabsBarPlugin {
    ...
    addListener(eventName: "accessoryPlayPause", listenerFunc: () => void): Promise<{
        remove: () => void;
    }>;
    /** Fires when user taps the bottom accessory body (open full player). */
    addListener(eventName: "accessoryTapped", listenerFunc: () => void): Promise<{
        remove: () => void;
    }>;
    /** Fires when accessory layout switches inline vs stacked. */
    addListener(eventName: "accessoryEnvironmentChanged", listenerFunc: (ev: TabAccessoryEnvironment) => void): Promise<{
        remove: () => void;
    }>;
}
